...
function TTraceNode.SetColor(Color: TColor): ITraceNode;
Var
   CommandList : TStringList;
begin
   result := self ;
   ...
   { send a second trace that is compatible with version 12 }

   CommandList := TStringList.create ; // will be freed by the thread
   addCommand (CommandList, CST_USE_NODE,id);             // param : the node that receive font change
   addCommand (CommandList, CST_BACKGROUND_COLOR, Color, inttostr(3)) ;      // param : color, colId
   TTrace.SendToWinTraceClient (CommandList, fWinTraceId);

   CommandList := TStringList.create ;
   addCommand (CommandList, CST_USE_NODE,id);             // param : the node that receive font change
   addCommand (CommandList, CST_BACKGROUND_COLOR, Color, inttostr(4)) ;      // param : color, colId
   TTrace.SendToWinTraceClient (CommandList, fWinTraceId);
end;
...
// ITraceNode
procedure TTraceNode.Indent (const leftMsg: string ; const RightMsg : string = '';
  const BackGroundColor: TColor = clBlack ; const IsEnter:boolean=false);
var
...
// ITraceNode
procedure TTraceNode.UnIndent (const leftMsg : string = '' ; const rightMsg : string = '';
  const BackGroundColor: TColor = clBlack ; const IsExit:boolean=false);
var
   CommandList : TStringList;
   pc : pchar ;
   leftMsg2, RightMsg2 : string ;
