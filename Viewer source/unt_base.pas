...
type
  ...
  private
    ...
    Offset : TPoint ;
  protected
    procedure AdjustPanelTop; virtual;
    procedure AdjustColumns(StartCol: Integer = 0); virtual;
  public
    { Public declarations }
...
//------------------------------------------------------------------------------
procedure TFrmBase.AdjustColumns(StartCol: Integer = 0);
var
  I: Integer;
begin
  AdjustPanelTop;
  if vst <> nil then
    if IsHighDPI then
    begin
      for I := StartCol to vst.Header.Columns.Count - 1 do
      begin
        if (I = 0) and (vst.Header.Columns[I].Width = 20) then
          vst.Header.Columns[I].Width := 25
        else
          vst.Header.Columns[I].Width :=
            Trunc(vst.Header.Columns[I].Width * 1.6);
      end;
      vst.Header.Height := vst.Header.Height + 9;
    end;
end;

procedure TFrmBase.AdjustPanelTop;
begin
