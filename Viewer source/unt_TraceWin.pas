...
interface
   uses
      ...
      Contnrs, unt_tool, math, printers,
...
uses
   ...
   begin
      ...
      AdjustColumns(0);  // Make sure vst has been set
      ApplyFont(); // set font name and size for the 2 trees (from XMLConfig)
      ApplyTheme();
      ShowLog(); // change the LabelLogFile caption

      // MOD_IL
      //vstTrace.Images  := Frm_Tool.vilActions16;
      PopupTree.Images := Frm_Tool.vilActions16;

      // AfterCellPaint is used to draw the trace level icon
      // and uses Frm_Tool.ImageList1
   end;
   // ------------------------------------------------------------------------------
         ...
         case Column of
            ...
            4: begin
                  if (TextType = ttNormal) and (IsSeparator(TreeRec.RightMsg))
                   ...
                   begin
                     if Length(TreeRec.RightMsg) > 400 then
   ...
   begin
      ...
      try
         ...
         // ----- normal case starts here -----
            ...
            begin
              if (TreeRec.Members = nil) or
                 ((TreeRec.Members <> nil) and
                   (TreeRec.Members.SubMembers.Count = 0)) then
              begin
                ...
                TFrame_XML(XmlFrame).AddComment(TreeRec.RightMsg);
              end
              else
         ...
         if TreeRec.Members <> nil then begin
            ...
            for c := 0 to TreeRec.Members.SubMembers.Count - 1 do begin
               ...
               case SubMember.ViewerKind of
                  CST_VIEWER_XML, CST_VIEWER_TXT: // xml viewer
                     begin
                        if XmlFrame = nil then begin
                           XmlFrame := TFrame_XML.Create(self);
                           // owner : self -> released by form
...
// ------------------------------------------------------------------------------
   ...
   procedure TFrm_Trace.AdjustPanelTop;
begin
  inherited;
  PanelTop.Height := PanelTop.Height + 8;
end;
...
// ------------------------------------------------------------------------------
   ...
   begin
     ...
     if TraceConfig.AppDisplay_DarkTheme then
       BrushColor := TStyleManager.ActiveStyle.GetSystemColor(clWindow);
      ...
      gutterCanvas.Brush.Color := BrushColor; // clgreen ; //clBtnFace ;
      ...
      gutterCanvas.Brush.Color := BrushColor;
      gutterCanvas.FillRect(CliRect);
   ...
   begin
      ...
      if (Column = 0) and (IsMultiColTree = false) then begin
         ...
         begin
           if TraceConfig.AppDisplay_DarkTheme then
             Frm_Tool.vilStatusW.Draw(TargetCanvas, 0, 0, ImageIndex-22)
           else
             Frm_Tool.vilStatus16.Draw(TargetCanvas, 0, 0, ImageIndex-22);
         end
         else
   ...
   var
      ...
      begin
        Result := FontDetail.BackgroundColor;
        if TraceConfig.AppDisplay_DarkTheme then
          Result := ConvertToDarkTheme(Result, 0.2);
      end;
   begin
      // then check for special font name, size and color
      Result := false;
      for c := 0 to length(FontDetails) - 1 do begin
         ...
         if (FontDetail.ColId = Column) or (FontDetail.ColId = -1) then begin
            ...
            if FontDetail.BackgroundColor <> 0 then
            begin
               TargetCanvas.Brush.Color := GetBackgroundColor;
   ...
   begin
      ...
      if TraceConfig.AppDisplay_DarkTheme then
      begin
        vstTrace.Color := TStyleManager.ActiveStyle.GetSystemColor(clWindow);
        vstTrace.Header.Columns[3].Options := vstTrace.Header.Columns[3].Options
          + [coParentColor, coStyleColor];
        vstTrace.Header.Columns[4].Options := vstTrace.Header.Columns[4].Options
          + [coParentColor, coStyleColor];

        vstTrace.Header.Columns[3].Color := TStyleManager.ActiveStyle.GetSystemColor(clWindow);
        vstTrace.Header.Columns[4].Color := TStyleManager.ActiveStyle.GetSystemColor(clWindow);
      end
      else
      begin
        vstTrace.Color := clWindow;
        vstTrace.Header.Columns[3].Options := vstTrace.Header.Columns[3].Options
          - [coParentColor, coStyleColor];
        vstTrace.Header.Columns[4].Options := vstTrace.Header.Columns[4].Options
          - [coParentColor, coStyleColor];

        vstTrace.Header.Columns[3].Color := $00FEE7EB;
        vstTrace.Header.Columns[4].Color := $00FEE7EB;
      end;
      if TreeDetailFrame <> nil then
        Tframe_Classic(TreeDetailFrame).ApplyTheme;
      if XmlFrame <> nil then
        TFrame_XML(XmlFrame).ApplyTheme;
   end;
