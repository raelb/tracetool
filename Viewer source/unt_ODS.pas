...
uses
   ...
   , unt_utility
   , unt_TraceConfig
   , unt_search, Vcl.Themes; //,Unt_Tool;
...
procedure TFrm_ODS.AdjustPanelTop;
begin
  inherited;
...
begin
  ...
  begin
    { vstDebugString }
    vstDebugString.Color := TStyleManager.ActiveStyle.GetSystemColor(clWindow);
    vstDebugString.Header.Columns[2].Options :=
      vstDebugString.Header.Columns[2].Options + [coParentColor, coStyleColor];

    vstDebugString.Header.Columns[2].Color := TStyleManager.ActiveStyle.GetSystemColor(clWindow);
    ...
    PanelGutter.Color := TStyleManager.ActiveStyle.GetSystemColor(clBtnFace);
  end
  else
  begin
    { vstDebugString }
    vstDebugString.Color := clWindow;
    vstDebugString.Header.Columns[2].Options :=
      vstDebugString.Header.Columns[2].Options - [coParentColor, coStyleColor];

    vstDebugString.Header.Columns[2].Color := $00FEE7EB;
...
begin
   ...
   if TraceConfig.AppDisplay_DarkTheme then
     BrushColor := TStyleManager.ActiveStyle.GetSystemColor(clBtnFace);
