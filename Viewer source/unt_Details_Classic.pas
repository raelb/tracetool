...
type
  Tframe_Classic = class(Tframe_BaseDetails)
    ...
    procedure VstDetailGetText(Sender: TBaseVirtualTree; Node: PVirtualNode;
      Column: TColumnIndex; TextType: TVSTTextType; var CellText: String);
    procedure VstDetailAfterCellPaint(Sender: TBaseVirtualTree;
      TargetCanvas: TCanvas; Node: PVirtualNode; Column: TColumnIndex;
      CellRect: TRect);
    procedure VstDetailKeyAction(Sender: TBaseVirtualTree; var CharCode: Word;
...
begin
  ...
  VstDetail.TreeOptions.PaintOptions :=
    TraceWin.vstTrace.TreeOptions.PaintOptions + [toShowTreeLines];
  // show tree lines in 'members' tree
  VstDetail.TreeOptions.SelectionOptions :=
    TraceWin.vstTrace.TreeOptions.SelectionOptions + [toExtendedFocus]
  ...
  VstDetail.TreeOptions.MiscOptions := TraceWin.vstTrace.TreeOptions.MiscOptions
    - [toEditable]
  // don't allow edition. Code is used to detect double click or F2 key
    - [toReportMode]; // Tree behaves like TListView in report mode.
  VstDetail.Colors.UnfocusedSelectionColor :=
    TraceWin.vstTrace.Colors.UnfocusedSelectionColor;
  VstDetail.Colors.UnfocusedSelectionBorderColor :=
    TraceWin.vstTrace.Colors.UnfocusedSelectionBorderColor;
...
// ------------------------------------------------------------------------------
procedure Tframe_Classic.VstDetailChange(Sender: TBaseVirtualTree;
  Node: PVirtualNode);
begin
  // scroll into view
...
begin
  ...
  if SelectedNode <> MouseNode then
    exit;
  VstDetail.TreeOptions.MiscOptions := VstDetail.TreeOptions.MiscOptions +
    [toEditable];
...
procedure Tframe_Classic.VstDetailEdited(Sender: TBaseVirtualTree;
...
begin
  try
    ...
    on e: exception do
    begin
      TFrm_Trace.InternalTrace('VstDetailFreeNode exception when resetting',
        e.Message);
    end;
  end;
end;
...
begin
  ...
  if (IsSeparator(CellText)) and (DetailRec.Col2 = '') and (DetailRec.Col3 = '')
  then
  begin
    CellText := '';
    // don't display any text. separtor is draw in VstDetailAfterCellPaint procedure
    exit;
  end;
  case Column of
    0:
      CellText := DetailRec.Col1;
    1:
      CellText := DetailRec.Col2;
    2:
      CellText := DetailRec.Col3;
  end;
end;
...
begin
  ...
  begin // mrYesToAll means Highlight all
    case Column of
      0:
        if (MatchSearch(DetailRec.Col1) <> 0) then
          DrawHighlight(TargetCanvas, CellRect, false);
      1:
        if (MatchSearch(DetailRec.Col2) <> 0) then
          DrawHighlight(TargetCanvas, CellRect, false);
      2:
        if (MatchSearch(DetailRec.Col3) <> 0) then
          DrawHighlight(TargetCanvas, CellRect, false);
    end;
...
begin
  ...
  begin
    ...
    middle := (CellRect.Bottom div 2) - 2;
    // remove 2 points to display the separator on the position as the horizontal tree line
...
begin
  ...
  DetailRec := Sender.GetNodeData(Node);
  // force font
  TraceWin.ChangeFontDetail( { trace } false, TargetCanvas, 0,
    DetailRec.FontDetails, True); // Watch/Framework , Trace/info
  NodeHeight := TVirtualStringTree(Sender).ComputeNodeHeight(TargetCanvas,
    Node, 0);
  TraceWin.ChangeFontDetail( { trace } false, TargetCanvas, 1,
    DetailRec.FontDetails, True);
  h2 := TVirtualStringTree(Sender).ComputeNodeHeight(TargetCanvas, Node, 1);
  TraceWin.ChangeFontDetail( { trace } false, TargetCanvas, 2,
    DetailRec.FontDetails, True);
...
begin
  DetailRec := Sender.GetNodeData(Node);
  // force font
  TraceWin.ChangeFontDetail( { IsTrace } false, TargetCanvas, Column,
    DetailRec.FontDetails, (vsSelected in Node.States));
end;
...
// ------------------------------------------------------------------------------
procedure Tframe_Classic.ApplyTheme;
var
  I: Integer;
begin
  inherited;
  ...
  begin
    VstDetail.Color := TStyleManager.ActiveStyle.GetSystemColor(clWindow);
    for I := 0 to VstDetail.Header.Columns.Count - 1 do
    begin
      VstDetail.Header.Columns[I].Options := VstDetail.Header.Columns[I].Options
          + [coParentColor, coStyleColor];

      VstDetail.Header.Columns[I].Color :=
        TStyleManager.ActiveStyle.GetSystemColor(clWindow);
    end;
  end
  else
  begin
    VstDetail.Color := $00F5EEE7;
    for I := 0 to VstDetail.Header.Columns.Count - 1 do
    begin
      VstDetail.Header.Columns[I].Options := VstDetail.Header.Columns[I].Options
          - [coParentColor, coStyleColor];
      VstDetail.Header.Columns[I].Color := $00F5EEE7;
    end;
  end;
end;
